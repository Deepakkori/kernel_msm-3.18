...
static bool inserted_headset_ptt = false;
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		//PTT Btn Changes start
		pinctrl_select_state(ptt_btn_pinctrl, ptt_btn_state_suspend);
		...
		gpio_set_value(mbhc->headset_sel, 1); // 56 to slect path
		gpio_set_value(mbhc->audio_sw, 1); // 58 enable switch high
		msleep(180);
		pr_err("%s:%d ptt btn state:%d\n",__func__,__LINE__,gpio_get_value(mbhc->ptt_btn));
		if (gpio_get_value(mbhc->ptt_btn)) {
			...
			gpio_set_value(mbhc->headset_sel, 0); // 56 to slect path
			inserted_headset_ptt = false;
		}
		//PTT Btn Changes end
		/*
...
//	pinctrl_select_state(ptt_btn_pinctrl, ptt_btn_state_active);
	mbhc->ptt_btn = of_get_named_gpio(card->dev->of_node, "qcom,ptt-btn-gpio", 0);
	if (gpio_is_valid(mbhc->ptt_btn))
		gpio_request_one(mbhc->ptt_btn, GPIOF_DIR_IN, "ptt_btn");

	mbhc->audio_sw = of_get_named_gpio(card->dev->of_node, "qcom,audio-switch-gpio", 0);
	if (gpio_is_valid(mbhc->audio_sw))
		gpio_request_one(mbhc->audio_sw, GPIOF_DIR_OUT | GPIOF_INIT_LOW, "audio_sw");
	mbhc->headset_sel = of_get_named_gpio(card->dev->of_node, "qcom,headset-sel-gpio", 0);
	if (gpio_is_valid(mbhc->headset_sel))
		gpio_request_one(mbhc->headset_sel, GPIOF_DIR_OUT | GPIOF_INIT_LOW, "headset_sel");
//PTT Btn Changes end
	...
	if (mbhc->headset_jack.jack == NULL) {
		...
		INIT_DELAYED_WORK(&mbhc->mbhc_btn_dwork, wcd_btn_lpress_fn);
                INIT_DELAYED_WORK(&mbhc->mbhc_ptt_btn_dwork, wcd_ptt_btn_fn);
	}
